...
from huaweisms.xml.util import (
    get_child_text,
    parse_xml_string,
    get_dictionary_from_children,
)
...
def get_from_url(url, ctx=None, additional_headers=None, timeout=None):
    ...
    r = get_from_url_raw(
        url=url, ctx=ctx, additional_headers=additional_headers, timeout=timeout
    )
    return api_response(r)


def get_from_url_raw(url, ctx=None, additional_headers=None, timeout=None):
    # type: (str, ApiCtx, dict, int) -> requests.Response
    cookies = build_cookies(ctx)
    headers = common_headers()
